...
/*   Updated: 2018/04/26 16:42:53 by blefeuvr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		cw_fork(t_vm *vm, t_process *process)
{
	...
	if (!(new = ft_lstnew(process, sizeof(t_process))))
		cw_error(ERR_MALLOC);
	((t_process*)new->content)->pc = (unsigned int)(old_pc + ((short)param
			% IDX_MOD)) % MEM_SIZE;
	...
	if (DEBUG)
		ft_printf("P    %d | fork %d (%d) P %d\n", process->id, param, \
				old_pc + ((short)param % IDX_MOD), vm->nb_process);
	return (1);
}
