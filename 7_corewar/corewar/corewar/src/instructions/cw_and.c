...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/04/24 15:02:33 by blefeuvr          #+#    #+#             */
/*   Updated: 2018/04/27 17:46:17 by blefeuvr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				cw_and_param(t_vm *vm, t_process *process, int param[3],
		int ret[3])
{
	int				i;
	int				off;
...
int				cw_and(t_vm *vm, t_process *process)
{
	...
	if ((cw_and_param(vm, process, param, ret)))
	{
		...
		if (DEBUG)
			ft_printf("P %d\nand: %d & %d (%d) -> r%d\n", process->id, param[0],
				param[1], param[0] & param[1], param[2]);
	}
	process->pc = (process->pc + 1) % MEM_SIZE;
	return (1);
}
